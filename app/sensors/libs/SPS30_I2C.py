...
class SPS30():
    ...
    def set_auto_cleaning_interval(self, seconds):
        # FIX: Create a copy instead of modifying the class list
        command = self.RW_AUTO_CLN.copy()
        command.append((seconds >> 24) & 0xFF)
        command.append((seconds >> 16) & 0xFF)

        command.append(calculateCRC(command[2:4]))

        command.append((seconds >> 8) & 0xFF)
        command.append(seconds & 0xFF)

        command.append(calculateCRC(command[5:7]))

        write = i2c_msg.write(self.SPS_ADDR, command)
        self.bus.i2c_rdwr(write)
